...
public class HiveSaveData extends WorldSavedData {
  ...
  public void readFromNBT(CompoundNBT nbt) {
    ListNBT list = nbt.getTagList("hive_data", NBT.TAG_COMPOUND);
  ...
  public void alertMembers(World world, Hive hive, UUID newMember, boolean joined) {
    ...
    if (members.isEmpty()) {
      PlayerEntity player = world.getPlayerEntityByUUID(hive.owner);
      if (player != null) {
        player.sendMessage(new TranslationTextComponent("arcanearchives.network.hive.disbanded", name).setStyle(new Style().setColor(TextFormatting.GOLD)));
      }
      return;
    }
    ...
    for (UUID uuid : members) {
      ...
      if (joined) {
        player.sendMessage(new TranslationTextComponent("arcanearchives.network.hive.joined_your_network", name).setStyle(new Style().setColor(TextFormatting.GOLD)));
      } else {
        player.sendMessage(new TranslationTextComponent("arcanearchives.network.hive.left_your_network", name).setStyle(new Style().setColor(TextFormatting.GOLD)));
      }
    }
  }
  ...
  public static class Hive {
    ...
    public void readFromNBT(CompoundNBT tag) {
      ...
      this.members.clear();
      ListNBT members = tag.getTagList("members", NBT.TAG_COMPOUND);
    ...
    @Nullable
    public static Hive fromNBT(CompoundNBT tag) {
