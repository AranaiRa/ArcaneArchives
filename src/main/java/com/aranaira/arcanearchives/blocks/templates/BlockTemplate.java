...
public class BlockTemplate extends Block implements IHasModel {
  ...
  private BlockItem itemBlock = null;
  ...
  public void setItemBlock(BlockItem itemBlock) {
  ...
  public ITextComponent getNameComponent() {
    return new TranslationTextComponent(String.format("%s.name", getTranslationKey()));
  }
    ...
    if (!world.isRemote) {
      ...
      if (this.hasAccessors() || this == BlockRegistry.LECTERN_MANIFEST) {
        for (BlockPos point : calculateAccessors(world, pos)) {
          world.setBlockState(point, this.getDefaultState().withProperty(ACCESSOR, true).withProperty(BlockDirectionalTemplate.FACING, Direction.fromAngle(placer.rotationYaw - 90)));
        }
      }
    }
  }
  ...
  public List<BlockPos> calculateAccessors(World world, BlockPos pos, @Nullable Direction facing) {
    ...
    Direction curOffset = Direction.fromAngle(facing.getHorizontalAngle() - ((size.width == size.length) ? 90 : 180));
