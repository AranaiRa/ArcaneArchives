...
public class CommandHive extends CommandBase {
	...
	public void execute (MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (sender instanceof EntityPlayer) {
			...
			if (hive.getOwner() == player.getUniqueID()) {
				player.sendMessage(new TextComponentString("You own this hive network."));
			} else {
				GameProfile profile = cache.getProfileByUUID(hive.getOwner());
				if (profile == null) {
			...
			player.sendMessage(new TextComponentString("UUID for network is: " + hive.getOwner().toString()));
			List<UUID> members = hive.getMembers();
			player.sendMessage(new TextComponentString("Hive has " + members.size() + " members."));
