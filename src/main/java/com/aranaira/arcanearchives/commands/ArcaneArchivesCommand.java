...
public class ArcaneArchivesCommand extends CommandBase
{
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		ServerNetwork network = NetworkHelper.getServerNetwork(eSender.getUniqueID(), server.getWorld(0));
		if(network == null) {
...
} else
{
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nonnull BlockPos targetPos) {
		if(args.length == 0) {
			...
			} else if (args[1].compareTo("accept") == 0 && sender.getCommandSenderEntity() != null) {    //TODO: Is this client or serverside? Client may not have network data for the below.
				ServerNetwork network = NetworkHelper.getServerNetwork(sender.getCommandSenderEntity().getUniqueID(), server.getWorld(0));
